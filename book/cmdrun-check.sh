...
# Currently, only calls to `gen_support_matrix_html.rs` and
# `gen_board_list_html.rs` will pass.
...
grep -qroP '(?s)<!--\s*cmdrun\s+(?!(\.\./\.\./doc/gen_support_matrix_html\.rs generate \.\./\.\./doc/support_matrix\.yml /dev/stdout --tier [123]|\.\./\.\./\.\./doc/gen_board_list_html\.rs \.\./\.\./\.\./doc/support_matrix\.yml /dev/stdout --tier [123])\s*-->).*?-->' src/
res=$?
...
# Check that we have the expected amount of allowed commands for `gen_support_matrix_html.rs`
...
# Check that we have the expected amount of allowed commands for `gen_board_list_html.rs`
test "$(grep -roP '<!--\s*cmdrun\s+\.\./\.\./\.\./doc/gen_board_list_html\.rs \.\./\.\./\.\./doc/support_matrix\.yml /dev/stdout --tier [123]\s*-->' src/ | wc -l)" -eq 3
