...
grep -qroP '(?s)<!--\s*cmdrun\s+(?!\.\./\.\./doc/gen_support_matrix_html\.rs generate \.\./\.\./doc/support_matrix\.yml /dev/stdout --tier [12]\s*-->).*?-->' src/
res=$?
...
test "$(grep -roP '<!--\s*cmdrun\s+\.\./\.\./doc/gen_support_matrix_html\.rs generate \.\./\.\./doc/support_matrix\.yml /dev/stdout --tier [12]\s*-->' src/ | wc -l)" -eq 2
