...
impl StreamState {
    ...
    /// Handle control flow commands received from the `command` channel, retruns what the main loop should do.
    ...
    /// Handles data from nrf-modem, retruns true if the stream processing should be stopped.
    pub fn handle_stream_data(
        ...
        let Some(data) = stream_data else {
            ...
            let next_action = if self.single_shot {
                ...
                self.latest_data
                    .map(|pos| NextAction::SendFinalData(pos))
                    .unwrap_or(NextAction::SendFinalError(ReadingError::SensorAccess))
            } else {
                NextAction::Break
            };
            return next_action;
    ...
    async fn handle_stream_events(
        ...
        loop {
            ...
            match next_action {
                ...
                NextAction::SendFinalData(pos) => self.send_data(&pos),
