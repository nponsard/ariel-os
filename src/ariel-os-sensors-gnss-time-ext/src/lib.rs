//! # GNSS Time Extension
//!
//! This extension to [ariel-os-sensors] allows you to read the time information from [`Samples`] produced
...
//!
//! - [`GnssTimeExt::time_of_fix_timestamp()`]: UTC time in whole seconds since the UNIX epoch.
//! - [`GnssTimeExt::time_of_fix_timestamp_nanos()`]: UTC time in nanoseconds since the UNIX epoch.
...
//! You need to return a [`ReadingChannel`] with the label [`Label::OpaqueGnssTime`] containing the first
//! part *directly followed* by a [`ReadingChannel`] with the label [`Label::Opaque`] containing the second part.
//!
//! [ariel-os-sensors]: ariel_os_sensors
...
pub enum GnssTimeExtError {
    ...
    Reading(SampleError),
    /// Sensor does not provide the correct [`ReadingChannels`](ariel_os_sensors::sensor::ReadingChannels).
    InvalidSensor,
}
...
/// Error returned when trying to convert the time to two parts.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
...
/// Trait to access time information from [`Samples`] coming from a GNSS sensor.
pub trait GnssTimeExt: private::Sealed {
    /// Returns the UTC time in whole seconds since the UNIX epoch.
    ///
    /// # Errors
    ///
    ...
    /// Returns the time of the last fix in nanoseconds since the UNIX epoch.
    ///
    /// # Errors
    ///
    /// Returns errors if the reading is not compatible with this extension
    ...
    fn time_of_fix_timestamp_nanos(&self) -> Result<i128, GnssTimeExtError> {
        Ok(
            i128::from(self.time_of_fix_timestamp()?) * 1_000_000_000i128
                + i128::from(self.time_of_fix_subsec_nanos()?),
        )
    }
    ...
    /// Returns errors if the reading is not compatible with this extension
    /// or one [`Sample`] returned an error.
    fn time_of_fix_subsec_nanos(&self) -> Result<i64, GnssTimeExtError>;
}
...
/// Converts time to parts to be put into [`Samples`].
///
/// `timestamp_nanos` is an UTC timestamp as nanoseconds since the UNIX epoch.
///
/// The first element of the tuple is to be inserted in a [`ReadingChannel`] that has the label
/// [`Label::OpaqueGnssTime`].
/// The second element must be inserted in a [`ReadingChannel`] right after the [`ReadingChannel`] of the first element and
/// that has the label [`Label::Opaque`].
///
/// You would return [`Samples`] like this:
///
///     # use ariel_os_sensors_gnss_time_ext::mock_sensor::MockSensor;
///     # static SENSOR: static_cell::StaticCell<MockSensor> = static_cell::StaticCell::new();
///     # use ariel_os_sensors_gnss_time_ext::{convert_datetime_to_parts, GnssTimeExtConvertError};
///     # use ariel_os_sensors::{sensor::{Samples, Sample, ReadingChannel, ReadingChannels}, Label};
///     // For demonstration purposes, In a normal implementation the Samples should be returned by
///     // `wait_for_reading()` implemented by the Sensor.
...
///     let (first, second) = convert_datetime_to_parts(timestamp_ns)?;
///     let samples = Samples::from_2(
///         sensor,
///         [
///             Sample::new(
///                 first,
...
fn get_element_after_marker(
    ...
    // if it's not an opaque channel we're doing something wrong
...
/// Mock sensor module for tests.
/// Has to be public for docs tests.
#[cfg(any(test, feature = "_test"))]
pub mod mock_sensor {
    use ariel_os_sensors::{Sensor, sensor::ReadingChannels};

    /// Mock sensor for tests.
    ...
    impl MockSensor {
        ...
        pub const fn new(reading_channels: ReadingChannels) -> Self {
...
#[cfg(test)]
mod tests {
    use ariel_os_sensors::sensor::ReadingChannels;

    use super::{mock_sensor::MockSensor, *};
    ...
    fn end_to_end() {
        ...
        assert_eq!(samples.time_of_fix_subsec_nanos().unwrap(), ns);
    ...
    fn end_to_end_invalid_sensor() {
        ...
        assert!(matches!(
            samples_wrong_order.time_of_fix_subsec_nanos(),
        ...
        assert!(matches!(
            samples_no_second_part.time_of_fix_subsec_nanos(),
            Err(GnssTimeExtError::InvalidSensor)
        ));
        assert!(matches!(
