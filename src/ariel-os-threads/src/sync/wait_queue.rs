...
impl WaitQueue {
    ...
    pub fn wait_until(&self, deadline: embassy_time::Instant) -> bool {
        ariel_os_log::trace!("WaitQueue::wait_until()");
        // Safety:
        ...
        unsafe {
            crate::timeout::with_deadline(
                ...
                |cs| {
                    ariel_os_log::trace!("WaitQueue::wait_until() timeout");
    ...
    pub fn wait_until_with_check(
        ...
        ariel_os_log::trace!("WaitQueue::wait_until_with_check()");
        // Safety:
        // `on_timeout` takes care of removing the thread from the threadlist.
        unsafe {
            crate::timeout::with_deadline_check(
                ...
                |cs| {
                    ariel_os_log::trace!("WaitQueue::wait_until_with_check() timeout");
    ...
    pub fn notify_one(&self) {
        critical_section::with(|cs| {
            ...
            ariel_os_log::trace!("WaitQueue::notify_one() notifying {:?}", res);
        });
    }
}
