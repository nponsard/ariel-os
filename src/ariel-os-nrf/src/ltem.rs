...
// `context::Control` is not `Send`, we can only interact with it in `control_task()`.
// Using a `Signal` here so we can disable/enable the LTE-M modem from any task/thread.
...
fn status_to_config(status: &Status) -> embassy_net::Config {
    ...
    if let Some(ref link) = status.ipv4_link {
        // We make sure `dns_servers` has a capacity superior or equal to the one of link.dns
        let mut dns_servers = can_contain(&link.dns);
